...
class Client: # Client's communication is synchronous: It can not send a request until its last request is answered.
    ...
    def broadcast_request(self,request_message,nodes_ids_list,sending_time,f): # This function is executed if the primary node doesn't receive the request. It is then broadcasted to all the nodes
        ...
        while True:
            ...
            settings.number_of_messages_sent_per_node[answering_node_id] +=1 
            
            if settings.mean_messages_size[answering_node_id]==0:
                    settings.mean_messages_size[answering_node_id]=received_message_size
            else:
                    settings.mean_messages_size[answering_node_id]=(settings.mean_messages_size[answering_node_id]+received_message_size)/2 # The mean
            ...
            if (digest != requests_digest):
                settings.number_of_faulty_messages_sent_per_node[answering_node_id] +=1 
            ...
            if (answering_node_id not in answered_nodes): # Ne rien faire si le noeud a déjà répondu
                    ...
                    if replies[str_response] >= (f+1):
                            ...
                            if similar_replies == (f+1):
                                ...
                                settings.number_of_validated_requests +=1

                                for i in range (settings.n):
                                    if i in (nodes_replies) and (nodes_replies[i]) == (accepted_response):
                                        settings.number_of_validated_blocks_per_node[i] += 1
                                        import math
                                        if (settings.mean_delay_to_validate_accepted_block_per_node[i] == math.inf):
                                            
                                            settings.mean_delay_to_validate_accepted_block_per_node[i]= answering_latency[i]                                        
                                        else:
                                            settings.mean_delay_to_validate_accepted_block_per_node[i] = (settings.mean_delay_to_validate_accepted_block_per_node[i]+answering_latency[i])/2 # The mean
